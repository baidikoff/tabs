...
class TabsView<InnerDelegate: TabsViewDelegate>: UIView {
    ...
    private func reloadData() {
        ...
        self.views = self.items.map {
            ...
            self.innerDelegate.tabButtonView(self, willDisplayView: view, withItem: $0)
        ...
        if let first = self.views.first, let item = self.items.first {
            ...
            self.widthConstraints.append(first.widthAnchor.constraint(equalToConstant: size.width).activated())
            self.leadingConstraints.append(first.leadingAnchor.constraint(equalTo: self.scrollView.leadingAnchor).activated())
        }

        zip(self.views, self.views.dropFirst()).forEach { first, second in
            guard let secondItem = self.item(for: second) else { return }
            let secondSize = self.innerDelegate.tabButtonView(self, sizeForItem: secondItem)
            ...
            self.widthConstraints.append(second.widthAnchor.constraint(equalToConstant: secondSize.width).activated())
            self.leadingConstraints.append(second.leadingAnchor.constraint(equalTo: first.trailingAnchor, constant: self.appearance.tabsInnerSpacing).activated())
        }
    ...
    private func view(for item: Item) -> View? {
